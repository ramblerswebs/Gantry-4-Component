...
function gantry_admin_render_edit_item($element) {
    if ($element->type == 'tips' && (isset($element->element['tab']) && (string) $element->element['tab'] != 'overview'))
                        ...
                        <div class="g4-column">
                            ...
                            <ul class="g4-tabs">
                                ...
                                foreach ($fieldSets as $name => $fieldSet) {
                                    ...
                                    $involved = 0;
                                    if ($name == 'assignment' && (!$user->authorise('core.edit', 'com_menu') || !$canDo->get('core.edit.state')))
                                        continue;
                                    array_push($panels, array("name" => $name, "height" => (isset($fieldSet->height)) ? $fieldSet->height : null));
                                    foreach ($fields as $fname => $field) {
                                        $position = $field->panel_position;

                                        if ($field->type != 'hidden' && $field->setinoverride && $field->variance)
                                            $involved++;
                                        if ($field->type == 'hidden')
                                            $position = 'hiddens';
                                        if (!isset($positions[$position][$name]))
                                            $positions[$position][$name] = array();
                                        array_push(
            ...
            <div class="g4-body">
                ...
                if (count($panels) > 0) {
                    for ($i = 0; $i < count($panels); $i++) {
                        $panel = $panels[$i]['name'];
                        if ($panel == 'assignment' && (!$user->authorise('core.edit', 'com_menu') || !$canDo->get('core.edit.state')))
                            continue;
                        $width = '';

                        // RA fix around if statement
                        $left = $positions['left'];
                        $right = $positions['right'];
                        $leftpanel = array_key_exists($panel, $left);
                        $rightpanel = array_key_exists($panel, $right);
                        ...
                        foreach ($positions as $name => $position) {
                            ...
                            if (isset($positions[$name][$panel])) {
                                ...
                                if ($panel != 'overview' && $name == 'right') {
                                    foreach ($positions[$name][$panel] as $element) {
                                        if (get_class($element) != 'GantryFormFieldTips')
                                            continue;
